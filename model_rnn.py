...
class VanillaRNNLM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor | None = None):
        b, t = x.shape
        e = self.emb(x)
        h = torch.zeros(b, self.w_hh.in_features, device=x.device) if h0 is None else h0
        hs = []
        for i in range(t):
            h = torch.tanh(self.w_ih(e[:, i]) + self.w_hh(h))
            hs.append(h)
        h_seq = torch.stack(hs, dim=1)
        logits = self.proj(h_seq)
        return logits
...
def sample_name(
    ...
    model.eval()
    h = None
    cur = torch.tensor([[vocab.bos]], device=device)
    out = []
    for _ in range(max_len):
        logits = model(cur, h0=h)
        last = logits[:, -1, :] / max(temperature, 1e-6)
        if top_k and top_k > 0:
            v, ix = torch.topk(last, k=min(top_k, last.size(-1)))
            probs = F.softmax(v, dim=-1)
            pick = ix[0, torch.multinomial(probs[0], 1)]
        else:
            probs = F.softmax(last, dim=-1)
            pick = torch.multinomial(probs[0], 1)
        token = int(pick.item())
        if token == vocab.eos:
            break
        if token not in (vocab.bos, vocab.pad):
            out.append(vocab.itos[token])
        cur = torch.cat([cur, pick.view(1, 1)], dim=1)
    return "".join(out)
...
def main():
    ...
    p.add_argument("--save", type=str, default="checkpoints/rnn.pt")
    args = p.parse_args()
    ...
    model = VanillaRNNLM(vocab.size, args.emb_dim, args.hidden).to(device)
