...
def sample_name(
    ...
    for step in range(max_len):
        ...
        logits = model(x)
        step_logits = logits[0, len(seq) - 1, :] / max(temperature, 1e-6)

        step_logits[vocab.pad] = -1e9
        if step < min_len:
            step_logits[vocab.eos] = -1e9

        if top_k and top_k > 0:
            v, ix = torch.topk(step_logits, k=min(top_k, step_logits.size(-1)))
...
def main():
    ...
    p.add_argument("--save", type=str, default="checkpoints/blstm.pt")
    args = p.parse_args()
    ...
    model = BLSTMLM(vocab.size, args.emb_dim, args.hidden).to(device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if ep in (1, args.epochs // 2, args.epochs):
            s = sample_name(model, vocab, device, future_window=args.future_window)
            print("sample:", s)
